...
# @torch.no_grad()
def run_inference(generator, device, content_im_path, style_im_path, 
                  compress=False, gen_steps=3, strength=1.0, crop=False):
    with torch.no_grad():
        generator.eval()
        ...
        base_im = gen_preprocess(content_im, crop).to(device)
        base_im = base_im.unsqueeze(0)
        base_e = generator(base_im).flatten()
        base_encoded_flow = NeuralODE(input_dim=3, hidden=generator.hidden, device=device)
        base_encoded_flow.set_weights(base_e)
        
        targ_im = gen_preprocess(style_im, crop).to(device)
        targ_im = targ_im.unsqueeze(0)
        targ_e = generator(targ_im).flatten()
        targ_encoded_flow = NeuralODE(input_dim=3, hidden=generator.hidden, device=device)
        ...
        latent_x = base_encoded_flow.sample(base_x, N=gen_steps, strength=strength)
        styled_x = targ_encoded_flow.inv_sample(latent_x, N=gen_steps, strength=strength)
...
def run_inference_flow(device, content_flow_path, target_flow_path, content_im_path, 
                       hidden=64, gen_steps=3, strength=1.0, compress=None):
    with torch.no_grad():                       
        content_im = Image.open(content_im_path).convert('RGB')
        ...
        latent_x = base_flow.sample(base_x, N=gen_steps, strength=strength)
        styled_x = targ_flow.inv_sample(latent_x, N=gen_steps, strength=strength)   
        latent_im = tensor_to_im(latent_x, w, h)
        styled_im = tensor_to_im(styled_x, w, h)
