...
from src.model.generator import Generator, gen_preprocess
from src.inference import print_images

from src.model.generator import INPUT_SIZE
...
SAVE = ['check_points/V7_generator/','_merged_8195_generator']

pretrained_path = SAVE[0] + "2024.04.28 14-08-55_merged_8195_generator_epoch_700000"
PRETRAINED_EPOCH = 700000


def get_generator_path(epoch):
...
print(f"k_dim for generator: {k_dim}")

generator = Generator(k_dim=k_dim, input_dim=4, hidden=1024, output_dim=3, device=device)
if pretrained_path:
...
print(f"DATA: {DATA}")
print(f"FLOW: {FLOW}")
print(f"savepath format: {get_generator_path(0)}")
...
for epoch in tt:
    ...
    im_gen = [gen_preprocess(im, rand_trans=True) for im in im_pil]
    im_flats = [im.reshape(INPUT_SIZE**2, 3) for im in im_gen]
    
    im_batch = torch.stack(im_gen, dim=0).to(device) # [32, 3, 256, 256]
    e = generator(im_batch)  # [32, 8195]
    ...
    v_pred = generator.apply_e(e, z_batch, t_batch)

    gen_loss = torch.mean((v_pred - v_batch)**2)

    loss_track.append(gen_loss.detach().cpu())
    tt.set_description(f"{epoch} || gen_loss: {loss_track[-1]}")
    tt.refresh()
    ...
    if epoch > 0 and epoch % 3000 == 0:
        ...
        torch.save(generator.state_dict(), get_generator_path(epoch))
        print(f'\n epoch: {epoch}  saved: {get_generator_path(epoch)}')
